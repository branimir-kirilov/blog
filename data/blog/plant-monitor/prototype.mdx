...
Next, I need to add the Adafruit libraries to Arduino IDE in order to control the OLED. Open Arduino IDE and click `Sketch -> Include Library -> Manage Libraries`. Search for Adafruit, scroll down to the Adafruit Gfx library, and install it. Next, scroll down to the `Adafruit SSD1306` library and install it.
...
After uploading, you should see the text "Hello world" on the screen.
...
the positive wire of the battery to the red probe on the multimeter and the negative (black) probe to the + of the ESP board. This should give me a reading of the amps the device uses and sure it does - during start
the whole thing uses **~150mA** and peaks up to **~250mA** while connecting to WiFi and sending data. This lasts about **~10 seconds**, after it, the device goes to deep-sleep. When this happens my multimeter shows no consumption at all,
...
If I run the calculator again, this time with the data without the display, the results are as expected entirely different - **~380 days**.
...
All of the above are just estimations so I decided to actually test how long would it last with the OLED on. The results were **terrible** - ~4 and a half days of measurements on 4h long intervals and the battery died!
